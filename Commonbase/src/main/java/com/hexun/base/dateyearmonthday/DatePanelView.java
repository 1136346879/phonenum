package com.hexun.base.dateyearmonthday;
...
public class DatePanelView extends LinearLayout {
    ...
    public static interface OnPanelListener {
         ...
         */
        public void onPanelClosed(DatePanelView panel);
        /**
         * Invoked when the panel becomes fully opened.
         */
        public void onPanelOpened(DatePanelView panel);
    }
    ...
    public DatePanelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.DatePanel);
        mDuration = a.getInteger(R.styleable.DatePanel_animationDuration, 750);		// duration defaults to 750 ms
        mPosition = a.getInteger(R.styleable.DatePanel_position, BOTTOM);			// position defaults to BOTTOM
        mLinearFlying = a.getBoolean(R.styleable.DatePanel_linearFlying, false);	// linearFlying defaults to false
        mOpenedHandle = a.getDrawable(R.styleable.DatePanel_openedHandle);
        mClosedHandle = a.getDrawable(R.styleable.DatePanel_closedHandle);
        a.recycle();
...
//	            }
    ...
    private void postProcess() {
        ...
        if (panelListener != null) {
            if (mIsShrinking) {
                panelListener.onPanelClosed(DatePanelView.this);
            } else {
                panelListener.onPanelOpened(DatePanelView.this);
            }
        }
    }
